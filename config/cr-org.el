...
(use-package org
  ...
  (prettify-symbols-alist '(("#+PROPERTY:" . "") ("#+property:" . "")
                            ...
                            ("#+begin_src" . "↦") ("#+BEGIN_SRC" . "↦")
                            ("#+end_src" . "⇤") ("#+END_SRC" . "⇤")))
  (org-capture-templates '(("f" "Freelance" entry (file+headline "~/org/freelance_syadem_todo.org" my/select-freelance-todo-healine)
                            "* TODO %?\n  %i\n  %l")
                           ("v" "Vie" entry (file+headline "~/org/vie_syadem_todo.org" my/select-vie-todo-healine)
                            "* TODO %?\n  %i\n  %l")
  ...
  (org-agenda-custom-commands
   '(("c" "Simple agenda view"
      ((agenda "" ((org-agenda-overriding-header "Week Todo")
                   (org-agenda-start-day "+1d")
                   (org-agenda-start-on-weekday nil)
                   (org-agenda-log-mode-items '(state))
                   (org-agenda-prefix-format "%-15:T%s%l")
       ...
       (agenda "" ((org-agenda-overriding-header "Today")
                   ...
                   (org-agenda-span 1)
                   (org-agenda-prefix-format "%-15:T%-12t%l")
                   (org-agenda-skip-function '(org-agenda-skip-entry-if 'todo))
                   (org-agenda-skip-timestamp-if-done t)
                   (org-agenda-start-with-log-mode 'only)
                   (org-agenda-log-mode-items '(closed clock))))
       (agenda "" ((org-agenda-overriding-header "Week Done")
                   (org-agenda-start-day "-8d")
                   (org-agenda-span 8)
                   (org-agenda-prefix-format "%-15:T%t%l")
                   (org-agenda-skip-function '(org-agenda-skip-entry-if 'scheduled 'deadline 'todo))
                   (org-agenda-skip-timestamp-if-done t)
                   (org-agenda-start-with-log-mode t)
